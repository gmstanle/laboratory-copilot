...
Provide a JSON with no more than 1 question.
...
try:
    ...
    print("Failed to decode JSON:", e)
    set_data = {}  # Handle the error by setting to an empty dictionary or suitable default
...
for step in set_data['experiment_steps']:
    ...
    for consumable in step['consumables_required']:
        ...
        if item in consumables_set:
          consumables_set[item] += quantity
        else:
          consumables_set[item] = quantity
...
final_prompt = '''Please provide a detailed experiment protocol from the following information using markdown format:
